...
<main id="main">
  <div class="block-inner">
    ...
    <div class="section">
      <?php echo '<form action="'. BASE .'/register/additional" method="post">'; ?>
        <?php if ($this->shouldAskAllergy($this->option)) { ?>
          <fieldset class="form-fieldset">
            <legend><?php echo $this->formCount() ?>.アレルギー</legend>
            <div class="form-description">
              <p>アレルギーがある方は入力してください。</p>
            </div>
            <div class="form-part">
              <label for="name">
                <?php echo $this->formTextInput(array('name' => 'allergy', 'placeholder' => 'アレルギー'), $this->option) ?>
              </label>
            </div>
          </fieldset>
        <?php } ?>
        ...
        <?php if ($this->shouldAskReason($this->option)) { ?>
          <fieldset class="form-fieldset">
            <legend><?php echo $this->formCount() ?>.新歓合宿に参加しない理由 <span class="form-required">*</span></legend>
            <div class="form-description">
              <p>前のページの「新歓合宿への参加/不参加」で「参加しない」を選択した人は、新歓合宿に参加しない理由を入力してください。</p>
            </div>
            <div class="form-part">
              <label for="ruby">
                <?php echo $this->formTextInput(array('name' => 'reason', 'placeholder' => '理由', 'textarea' => true), $this->option) ?>
              </label>
            </div>
          </fieldset>
        <?php } ?>
        ...
        <fieldset class="form-fieldset">
          <legend><?php echo $this->formCount() ?>.その他</legend>
          <div class="form-description">
            <p>新歓合宿や懇親会に関する質問を入力してください。</p>
          </div>
          <div class="form-part">
            <label for="email">
              <?php echo $this->formTextInput(array('name' => 'question', 'placeholder' => '質問等', 'textarea' => true), $this->option) ?>
            </label>
          </div>
        </fieldset>
